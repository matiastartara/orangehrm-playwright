# 🎭 Storage Login — Playwright E2E Tests

E2E test automation project built with **Playwright** and **TypeScript**, implementing the **Page Object Model (POM)** pattern and **Storage State** to reuse the authenticated session across tests — avoiding repeated login flows on every run.

The application under test (AUT) is [OrangeHRM Live Demo](https://opensource-demo.orangehrmlive.com/).

---

## 🛠️ Tech Stack

| Tool | Version |
|---|---|
| [Playwright](https://playwright.dev/) | `^1.61.1` |
| [TypeScript](https://www.typescriptlang.org/) | `^6.0.3` |
| Node.js | `>=18` recommended |

---

## 🚀 Getting Started

### 1. Clone the repository

```bash
git clone https://github.com/<your-username>/storage-login-playwright.git
cd storage-login-playwright
```

### 2. Install dependencies

```bash
npm install
```

### 3. Install Playwright browsers

```bash
npx playwright install
```

> To install only Chromium for a minimal setup:
> ```bash
> npx playwright install chromium
> ```

---

## ▶️ Running the Tests

### Run all tests (setup + e2e)

```bash
npx playwright test
```

### Run only E2E tests (assumes `.auth/user.json` already exists)

```bash
npx playwright test --project=e2e-chromium
```

### Run only the authentication setup

```bash
npx playwright test --project=setup
```

### Run a specific test file

```bash
npx playwright test tests/e2e/login.spec.ts
npx playwright test tests/e2e/userSearch.spec.ts
npx playwright test tests/e2e/directorySearch.spec.ts
```

### Run in UI mode (interactive visual runner)

```bash
npx playwright test --ui
```

### Run in headed mode (watch the browser)

```bash
npx playwright test --headed
```

---

## 📊 Test Report

After each run, Playwright generates an HTML report. Open it with:

```bash
npx playwright show-report
```

---

## 🔐 Authentication Strategy (Storage State)

This project uses **Storage State** to persist the user session:

1. The `setup` project runs `tests/auth.setup.ts`, which performs the login on OrangeHRM and saves the session state (cookies and localStorage) to `.auth/user.json`.
2. The `e2e-chromium` project loads that state before each test, skipping the login step entirely.
3. Specific test specs (such as `tests/e2e/login.spec.ts`) override the global `storageState` with `undefined` to perform authentication testing in an unauthenticated browser context.

> The `.auth/user.json` file is auto-generated and included in `.gitignore`.

---

## 📁 Project Structure

```
storage-login-playwright/
│
├── .auth/                          # Generated session state (gitignored)
│   └── user.json                   # Storage state with cookies and localStorage
│
├── .agents/                        # Agent configuration and project conventions
│   └── AGENTS.md                   # Context and rules for AI coding agents
│
├── .github/                        # GitHub Actions CI configuration
│
├── pages/                          # Page Object Model (POM)
│   ├── BasePage.ts                 # Abstract base class with shared methods
│   ├── LoginPage.ts                # Page Object for Login page and authentication actions
│   ├── AdminPage.ts                # Page Object for the Admin > User Management section
│   ├── DirectoryPage.ts            # Page Object for the Directory section
│   ├── MenuPage.ts                 # Page Object for the main navigation sidebar
│   ├── PimPage.ts                  # Page Object for the PIM section and Add Employee form
│   └── PersonalDetailsPage.ts     # Page Object for the Personal Details employee page
│
├── tests/
│   ├── api/
│   │   └── api.spec.ts             # API REST tests (PIM, Admin, Directory, Validations)
│   ├── auth.setup.ts               # Setup: login and storage state persistence
│   └── e2e/
│       ├── add-employee.spec.ts    # E2E test: add employee in PIM and edit Personal Details
│       ├── login.spec.ts           # E2E test: login flows and unauthenticated checks
│       ├── user-search.spec.ts     # E2E test: user search in the Admin panel
│       └── directory-search.spec.ts # E2E test: employee search in the Directory
│
├── playwright-report/              # HTML report generated by Playwright
├── test-results/                   # Artifacts from failed tests (screenshots, traces)
│
├── playwright.config.ts            # Main Playwright configuration
├── tsconfig.json                   # TypeScript configuration
├── package.json                    # Dependencies and scripts
└── .gitignore
```

---

## ⚙️ Playwright Projects

| Project | Description |
|---|---|
| `setup` | Performs login and saves the Storage State |
| `e2e-chromium` | Runs E2E tests on Chrome using the saved session |
| `e2e-login` | Runs authentication tests in isolated clean context |
| `api` | Runs REST API tests; authenticates via HTTP requests, no browser involved |

E2E tests depend on the `setup` project, so the login step runs automatically first. The `api` project authenticates itself over HTTP and does not depend on `setup`.

---

## 🧩 Page Objects

### `BasePage`
Abstract base class extended by all Page Objects. Exposes reusable methods:
- `waitForPageLoad()` — waits for the `domcontentloaded` event
- `getTitle()` — returns the page title
- `reload()` — reloads the page and waits for it to load

### `LoginPage`
Page Object for the **Login** page and authentication flows. Provides:
- `navigateToLogin()` — navigates directly to the login URL
- `forceLogout()` — logs out current session and redirects to login
- `login(username, password)` — fills credentials and submits the login form
- Exposes locators for input fields, buttons, dashboard heading, and error messages (`invalidCredentialsMessage`, `requiredErrorMessages`)

### `AdminPage`
Page Object for the **Admin > User Management** section. Provides:
- `completeUsername(username)` — fills in the Username field
- `selectRole(role)` — selects a role from the dropdown
- `completeEmployeeName(name)` — fills and selects the employee name via autocomplete (waits for the API response)
- `setStatus(status)` — selects the user status
- `search()` — clicks the Search button
- `getRowByUsername(username)` — returns the row matching the given username
- `getCellFromRow(row, columnIndex)` — returns a specific cell from a row

### `DirectoryPage`
Page Object for the **Directory** section. Provides:
- `setJobTitle(title)` — selects a job title from the dropdown
- `setLocation(location)` — selects a location from the dropdown
- `search()` — clicks the Search button and waits for the API response
- `getCardResultSize()` — returns the total number of result cards
- `getCardResultAt(index)` — returns the card locator at a given index
- `getCardHeaderAt(index)` — returns the text of the card header at a given index

### `MenuPage`
Page Object for the **main navigation sidebar**. Centralizes sidebar navigation so tests don't hardcode `getByRole('link')` calls directly. Provides:
- `navigateToAdminPage()` — clicks the Admin sidebar link
- `navigateToDirectoryPage()` — clicks the Directory sidebar link
- `navigateToPimPage()` — clicks the PIM sidebar link and waits for the API response

### `PimPage`
Page Object for the **PIM** section. Provides:
- `clickAdd()` — clicks the Add Employee button
- `fillEmployeeName(firstName, middleName, lastName)` — fills the First, Middle, and Last Name fields
- `save()` — submits the Add Employee form

### `PersonalDetailsPage`
Page Object for the **Personal Details** page. Provides:
- `fillFirstSubsectionDetails(otherId)` — fills fields in the first subsection (such as Other Id)
- `savePersonalDetails()` — clicks the Save button for the first subsection
- `successToast` — locator for the green success notification toast
